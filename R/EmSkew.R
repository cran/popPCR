...
obj<-.C('emskewfit1',PACKAGE="popPCR",
...
obj<-.C('emskewfit2',PACKAGE="popPCR",
as.double(dat),as.integer(n),as.integer(p),
as.integer(g),as.integer(ncov),as.integer(ndist),#6
...
obj<-.C('mahalonobis_',PACKAGE="popPCR",
...
intraobj<-.C('intradist_',PACKAGE="popPCR",
...
interobj<-.C('interdist_',PACKAGE="popPCR",
as.double(dat),as.integer((n=nrow(dat))),as.integer((m=ncol(dat))),
as.integer(g),as.integer(clust),as.double(sigma),as.double(tau),
...
obj <- .Fortran('inverse3',PACKAGE="popPCR",
...
{stop("error: length not equal")}

if( (g=length(table(clust1)))!=length(table(clust2)))
{stop("the number of clusters are not equal")}
